...
joinCellExcept :: (Data e, Finite e, Monad m) => CellExcept a b (PortMidiT m) e -> CellExcept a b (HandlingStateT m) (Either e EOLCPortMidiError)
joinCellExcept = try . hoistCell squash . runCellExcept . fmap Left . hoist (withExceptT Right . unPortMidiT)
